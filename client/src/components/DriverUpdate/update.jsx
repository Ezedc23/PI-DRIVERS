...
function Create() {
	...
	const handleSubmit = (e) => {
		...
		if (
			drivers.some(
				(driver) =>
					driver.forename.toLowerCase() ===
						newDriver.forename.toLowerCase() &&
					driver.surname.toLowerCase() ===
						newDriver.surname.toLowerCase()
			)
		) {
			setErrors((prevErrors) => ({
				...prevErrors,
		...
		if (errors.ok) {
			...
			axios
				.post("http://localhost:3001/drivers", formattedDriver)
				.then((response) => {
					setErrors((prevErrors) => ({
						...prevErrors,
						ok: false,
	...
	return (
		<div className={styles.container}>
			...
			<div className={styles.sidebar}>
				<h2 style={{ marginTop: "5px", color: "red" }}>
					Create New Driver {errors.ok}
				</h2>
				...
				<form className={styles.form}>
					...
					<div className={styles.formField}>
						...
						<input
							...
							name="forename"
							type="text"
							defaultValue={newDriver.forename}
							onChange={handleChangeInput}
						/>
						{errors.forename ? (
							<span
								className={styles.errorIcon}
								title={errors.forename}
							>
								{"\u274C"}
							</span>
						) : (
						...
						<input
							style={{ width: "120px" }}
							name="surname"
							defaultValue={newDriver.surname}
							type="text"
							onChange={handleChangeInput}
						/>
						{errors.surname ? (
					...
					<div className={styles.formField}>
						...
						<input
							style={{ width: "75px" }}
							defaultValue={newDriver.nationality}
							name="nationality"
							type="text"
							onChange={handleChangeInput}
						/>
					...
					<div className={styles.formField}>
						...
						<textarea
							...
							defaultValue={newDriver.description}
							name="description"
							cols="100"
							onChange={handleChangeInput}
						/>
					...
					<div className={styles.formField}>
						<label
							...
							style={{ marginLeft: "-46px", marginTop: "23px" }}
						>
							Teams:
						</label>
						<select
					...
					<div>
						...
						{!selectedTeam.length ? (
							<span
								className={styles.errorIcon}
								title="No teams selected, one required"
							>
								{"\u274C"}
							</span>
						) : (
					...
					<div className={styles.formField}>
						<div style={{ display: "inline-block" }}>
							<button
								...
								disabled={
									!errors.ok || selectedTeam.length === 0
								}
