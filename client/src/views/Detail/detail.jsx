		...
		<div className={styles.detailContainer}>
			...
			{driver.createInDb ? (
				<button className={styles.deleteButton} onClick={deleteHandler}>
					<span
						title="Delete the Driver from the database"
						role="img"
						aria-label="Foto"
				...
				<button className={styles.deleteButton} disabled>
					<span
						title="Cannot delete, belongs to API"
						role="img"
						aria-label="Foto"
						className={styles.imgIcon}
					>
			...
			{driver && Object.keys(driver).length !== 0 ? (
				<>
					...
					{driver.name ? (
						...
						>{`${driver.name.forename} ${driver.name.surname}`}</h3>
					) : (
						<h3>{`${driver.forename} ${driver.surname}`}</h3>
					)}
					<h5
						className={styles.nacionalidad}
					>{`${driver.nationality}`}</h5>
					...
					{driver.description ? (
						...
						>{`${driver.description}`}</h5>
					) : (
						<h5
							className={styles.descripcion}
						>{`This driver has no description`}</h5>
					)}

					<h5>{`${driver.dob}`}</h5>
					{driver.teams ? (
						<h5 className={styles.teams}>
							{formatTeams(driver.teams)}
						</h5>
					) : (
						driver.Teams && (
							<h5 className="teams">
